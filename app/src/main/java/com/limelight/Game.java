...
public class Game extends ComponentActivity implements SurfaceHolder.Callback,
    ...
    protected TextView notificationOverlayView;
    private int requestedNotificationOverlayVisibility = View.GONE;
    protected TextView performanceOverlayView;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        NeuronBridge.updatePreferenceConfiguration(getWindow(), prefConfig, getIntent());
        tombstonePrefs = Game.this.getSharedPreferences("DecoderTombstone", 0);

        // Enter landscape unless we're on a square screen
        setPreferredOrientationForCurrentDisplay();

        if (isFullscreenLayout && (prefConfig.stretchVideo || shouldIgnoreInsetsForResolution(prefConfig.width, prefConfig.height))) {
    ...
    private final Runnable hideSystemUi = new Runnable() {
            @Override
            public void run() {
                // don't change system  UI if we are set to not change window flags
                if(!isFullscreenLayout) {
                    return;
                }
    ...
    private void stopConnection(Boolean isForceStop) {
        LimeLog.info("stopConnection: isForceStop="+isForceStop+", connecting="+connecting+", connecting="+connecting+", attemptedConnection="+attemptedConnection);
        if (isForceStop || connecting || connected || attemptedConnection) {
            ...
            new Thread() {
                public void run() {
                    LimeLog.info("stopConnection: Calling conn.stop");
                    conn.stop();
                    LimeLog.info("stopConnection: success");
                }
            }.start();
        }
    }
        ...
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                if (!displayedFailureDialog) {
                    ...
                    if (portFlags != 0) {
                        String neuronStringifyPortFlags = NeuronBridge.stringifyPortFlags(portFlags);
                        dialogText += "\n\n" + getResources().getString(R.string.check_ports_msg) + "\n" +
                                (neuronStringifyPortFlags == null ? MoonBridge.stringifyPortFlags(portFlags, "\n") : neuronStringifyPortFlags);
                    }
    ...
    /**
     * true if the message was handled (shown to user)
     * @param title
     * @param message
     * @return
     */
    ...
    protected void onSurfaceDestroyedWhileConnecting() {
        LimeLog.warning("onSurfaceDestroyedWhileConnecting:");
    }
    ...
    public void surfaceChanged(SurfaceHolder holder, int format, int width, int height) {
        LimeLog.info("surfaceChanged "+width+"x"+height);
        if (!surfaceCreated) {
    ...
    public void surfaceDestroyed(SurfaceHolder holder) {
        if(conn != null) {
            if(conn.isStarting()) {
                onSurfaceDestroyedWhileConnecting();
            }
        }
        if (!surfaceCreated) {
