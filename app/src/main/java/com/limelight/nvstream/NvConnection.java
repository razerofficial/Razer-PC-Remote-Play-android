...
public class NvConnection {
    ...
    public void stop() {
        ...
        if(startAppNvHTTP != null) {
            LimeLog.info("stop: closing startAppNvHTTP "+startAppNvHTTP.hashCode());
        ...
        synchronized (MoonBridge.class) {
            LimeLog.info("stop: stopConnection and cleanupBridge");
    ...
    private boolean startApp(final MediaCodecDecoderRenderer videoDecoderRenderer) throws XmlPullParserException, IOException
    {
        ...
        LimeLog.info("startApp: assigning startAppNvHTTP with "+(h.hashCode()));
        startAppNvHTTP = h;
        ...
        new Thread(new Runnable() {
            ...
            private void runImpl() {
                ...
                try {
                    ...
                    if(!isStarting) {
                        throw new ConnectionAlreadyStoppedException("Connection already stopped", null);
                    }
                    LimeLog.warning("start: stageComplete");
                    context.connListener.stageComplete(appName);
                } catch (HostHttpResponseException e) {
                    LimeLog.warning("start: error "+e.getMessage());
                    e.printStackTrace();
                    context.connListener.displayMessage(NeuronBridge.getUserFacingMessage(e));
                ...
                synchronized (MoonBridge.class) {
                    if(!isStarting) {
                        /**
                         * Connection already stopped, we should not call {@link MoonBridge#setupBridge(VideoDecoderRenderer, AudioRenderer, NvConnectionListener)}
                         * or continue with the connection because it will set an old {@link videoDecoderRenderer} that is probably already removed (Activity removed)
                         */
                        LimeLog.warning("start: Should not call setupBridge since connection already stopped. ");
                        connectionAllowed.release();
                        return;
                    }
    ...
    public String getLastLaunchQuery(){
        return startAppNvHTTP.getLastLaunchQuery();
    }
}
