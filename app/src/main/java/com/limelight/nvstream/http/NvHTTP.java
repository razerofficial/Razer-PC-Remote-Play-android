...
public class NvHTTP {
    ...
    private String lastLaunchUrl;

    private static String DEFAULT_DEVICE_NAME = RemotePlayConfig.getDefault().getDefaultDeviceName();
    private static int DEFAULT_HTTPS_PORT = RemotePlayConfig.getDefault().getDefaultHttpsPort();
    public static int DEFAULT_HTTP_PORT = RemotePlayConfig.getDefault().getDefaultHttpPort();
    ...
    private void initializeHttpState(final LimelightCryptoProvider cryptoProvider) {
        keyManager = new X509KeyManager() {
            public String chooseClientAlias(String[] keyTypes,
                    Principal[] issuers, Socket socket) { return RemotePlayConfig.getDefault().getX509ClientAlias(); }
    ...
    public NvHTTP(ComputerDetails.AddressTuple address, int httpsPort, String uniqueId, X509Certificate serverCert, LimelightCryptoProvider cryptoProvider) throws IOException {
        ...
        this.uniqueId = RemotePlayConfig.getDefault().getDefaultUniqueId();
    ...
    /**
     * Attempt to close shutdown all OkHttpClient connection to remove the timeout
     */
    public void close() {
        close(httpClientLongConnectNoReadTimeout);
        close(httpClientShortConnectTimeout);
    }

    private void close(OkHttpClient client) {
        try {
    ...
    public static String getXmlString(String str, String tagname, boolean throwIfMissing) throws XmlPullParserException, IOException {
    ...
    public ComputerDetails getComputerDetails(String serverInfo) throws IOException, XmlPullParserException {
        ...
        details.serverVersion = getServerVersion(serverInfo);
        details.activeDisplayMode = DisplayMode.getFirstActiveDisplayMode(serverInfo);
        String machineIdentifier = getMachineIdentifier(serverInfo);
    ...
    public String getMachineIdentifier(String serverInfo) throws XmlPullParserException, IOException {
        return getXmlString(serverInfo, "MachineIdentifier", false);
    }
    ...
    String executePairingCommand(String additionalArguments, boolean enableReadTimeout) throws HostHttpResponseException, IOException {
        return openHttpConnectionToString(enableReadTimeout ? httpClientLongConnectTimeout : httpClientLongConnectNoReadTimeout,
                baseUrlHttp, "pair", "devicename="+DEFAULT_DEVICE_NAME+"&updateState=1&" + additionalArguments+NeuronBridge.getPairQueryParameters());
    }
    ...
    String executePairingChallenge() throws HostHttpResponseException, IOException {
        return openHttpConnectionToString(httpClientLongConnectTimeout, getHttpsUrl(true),
                "pair", "devicename="+DEFAULT_DEVICE_NAME+"&updateState=1&phrase=pairchallenge"+NeuronBridge.getPairQueryParameters());
    }
    ...
    /**
     * @return The last query string used by {@link #launchApp}
     */
    public String getLastLaunchQuery() {
