...
public class ComputerDatabaseManager {
    ...
    private boolean addColumnIfNotExist(@NonNull String tableName, @NonNull String columnName, @NonNull String columnType) {
        try {
            computerDb.execSQL(String.format((Locale)null,"ALTER TABLE %s ADD COLUMN %s %s", tableName, columnName, columnType));
            LimeLog.info("Added new column "+tableName+"."+columnName+" "+columnType);
            return true;
        } catch (Exception e) {
    ...
    public boolean updateComputer(ComputerDetails details) {
        ...
        // machineIdentifier is not allowed to change, so it is ok to only update if it is not null
